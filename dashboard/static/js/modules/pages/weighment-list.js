...
class WeighmentList {
    ...
    constructor() {
        this.tbody       = domUtils.getElement('#weighmentTableBody');
    ...
    bindEvents() {
        ...
        domUtils.delegate('#weighmentTableBody', 'click', '.action-dropdown button', function(e, target) {
            e.preventDefault();
            e.stopPropagation();
    ...
    async loadData() {
        ...
        if (this.tbody) {
            this.tbody.innerHTML = `
                <tr><td colspan="10" class="text-center py-5 text-muted" style="font-size:13px;">
        ...
        try {
            ...
            const response = await apiClient.get('/api/weighment/', { params });
            const data = response;
            ...
            const totalDisplay = domUtils.getElement('#totalWeighmentDisplay');
            if (totalDisplay) totalDisplay.textContent = totalCount;

        } catch (err) {
            console.error('Weighment list load error:', err);
            if (this.tbody) {
                this.tbody.innerHTML = `
                    <tr><td colspan="10" class="text-center py-5 text-danger fw-bold fs-6">
    ...
    renderTable(records) {
        ...
        if (!records.length) {
            this.tbody.innerHTML = `
                <tr><td colspan="10" class="text-center py-5 text-muted" style="font-size:13px;">
                    <i class="bi bi-inbox me-2"></i>No Weighment records found.
                </td></tr>`;
            return;
        }
        ...
        this.bindRowActions();
        if (window.erpRowRestore && window.erpRowRestore.restore) {
            window.erpRowRestore.restore();
        }
    }

    renderRow(row) {
        const slipNo      = row.WeighmentSlipNo || '—';
        const gatePassNo  = row.GatePassNo != null ? `GP-${10000 + parseInt(row.GatePassNo)}` : '—';
        const date        = row.GrossDateTime ? formatter.formatDate(row.GrossDateTime) : '—';
        const vehicleType = row.VehicleType || '—';
        const grossWt     = row.GrossWeight != null ? parseFloat(row.GrossWeight).toFixed(2) : '—';
        const tareWt      = row.TareWeight  != null ? parseFloat(row.TareWeight).toFixed(2)  : '—';
        ...
        const editUrl = `/weighment/${encodeURIComponent(slipNo)}/edit/`;
        const viewUrl = `${editUrl}?mode=view`;

        const showDelete = (statusCode === 1 || statusCode === 3);
        const deleteItemHtml = showDelete ? `
                        <li><a class="dropdown-item py-1 text-warning weighment-delete-btn" href="#"
                            data-slip-no="${slipNo}">
                            <i class="bi bi-x-circle me-2 text-warning"></i>Mark Deleted
                        </a></li>` : '';

        return `
        <tr style="font-size: 12px; cursor: pointer;" data-row-id="${slipNo}">
            <td class="text-nowrap fw-semibold" style="color:#1d4ed8;">${slipNo}</td>
            <td>${gatePassNo}</td>
            <td class="text-nowrap">${date}</td>
            <td>${vehicleType}</td>
            <td class="text-end">${grossWt}</td>
            <td class="text-end">${tareWt}</td>
            <td class="text-end">${netWt}</td>
            <td style="max-width:120px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;" title="${purchaser}">${purchaser}</td>
    ...
    bindRowActions() {
        document.querySelectorAll('.weighment-delete-btn').forEach(btn => {
            btn.addEventListener('click', (e) => {
                e.preventDefault();
                const slipNo = btn.dataset.slipNo;

                const modalEl = document.getElementById('deleteConfirmModal');
                if (modalEl) {
                    const modal = bootstrap.Modal.getOrCreateInstance(modalEl);
                    const slipNoSpan = document.getElementById('deleteModalSlipNo');
                    if (slipNoSpan) slipNoSpan.textContent = slipNo;
                    ...
                    newConfirmBtn.addEventListener('click', async () => {
                        ...
                        try {
                            await apiClient.delete(`/api/weighment/${encodeURIComponent(slipNo)}/`);
                            notifications.success('Weighment Slip deleted successfully.');
                            modal.hide();
                            this.loadData();
                        } catch (err) {
...
document.addEventListener('DOMContentLoaded', () => new WeighmentList());
