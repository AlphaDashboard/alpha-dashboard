...
class GRNList {
    ...
    constructor() {
        this.tbody       = domUtils.getElement('#grnTableBody');
    ...
    bindEvents() {
        ...
        // Detached dropdowns delegation (Subsection B style)
        domUtils.delegate('#grnTableBody', 'click', '.action-dropdown button', function(e, target) {
            e.preventDefault();
            e.stopPropagation();
    ...
    async loadData() {
        ...
        try {
            ...
            const response = await apiClient.get('/api/grn/', { params });
            const data = response;
            ...
            const totalDisplay = domUtils.getElement('#totalGRNDisplay');
            if (totalDisplay) totalDisplay.textContent = totalCount;

        } catch (err) {
            console.error('GRN list load error:', err);
            if (this.tbody) {
                this.tbody.innerHTML = `
    ...
    renderTable(records) {
        ...
        if (!records.length) {
            this.tbody.innerHTML = `
                <tr><td colspan="9" class="text-center py-5 text-muted" style="font-size:13px;">
                    <i class="bi bi-inbox me-2"></i>No GRN records found.
                </td></tr>`;
            return;
        }
    ...
    renderRow(row) {
        ...
        const grnDate    = row.GrnDate  ? formatter.formatDate(row.GrnDate) : '—';
        const gatepass   = row.GatepassNo != null ? row.GatepassNo : '—';
        const netWt      = row.Netweight       != null ? parseFloat(row.Netweight).toFixed(2)      : '—';
        const dedWt      = row.DeductedWeight  != null ? parseFloat(row.DeductedWeight).toFixed(2) : '—';
        const appWt      = row.Approvedweight  != null ? parseFloat(row.Approvedweight).toFixed(2) : '—';
        ...
        const editUrl = `/grn/${encodeURIComponent(grnNo)}/edit/`;
        const viewUrl = `${editUrl}?mode=view`;

        const showDelete = (statusCode === 1 || statusCode === 3);
        const deleteItemHtml = showDelete ? `
                        <li><a class="dropdown-item py-1 text-warning grn-delete-btn" href="#"
                            data-grn-no="${grnNo}">
                            <i class="bi bi-x-circle me-2 text-warning"></i>Mark Deleted
                        </a></li>` : '';

        return `
        <tr style="font-size: 12px; cursor: pointer;" data-row-id="${grnNo}">
            <td class="text-nowrap fw-semibold" style="color:#1d4ed8;">${grnNo}</td>
            <td class="text-nowrap">${grnDate}</td>
            <td>${gatepass}</td>
            <td class="text-end">${netWt}</td>
            <td class="text-end">${dedWt}</td>
            <td class="text-end">${appWt}</td>
            <td style="max-width:180px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;" title="${notes}">${notes}</td>
    ...
    /**
     * Binds click event handlers for each row's action dropdown items.
     * Hooks up the 'Mark Deleted' button to open a custom Bootstrap confirmation
     * modal before calling the backend DELETE endpoint on confirmation.
     */
    bindRowActions() {
        document.querySelectorAll('.grn-delete-btn').forEach(btn => {
            btn.addEventListener('click', (e) => {
                e.preventDefault();
                const grnNo = btn.dataset.grnNo;
                
                const modalEl = document.getElementById('deleteConfirmModal');
                if (modalEl) {
                    const modal = bootstrap.Modal.getOrCreateInstance(modalEl);
                    const grnNoSpan = document.getElementById('deleteModalGrnNo');
                    if (grnNoSpan) grnNoSpan.textContent = grnNo;
                    ...
                    newConfirmBtn.addEventListener('click', async () => {
                        ...
                        try {
                            await apiClient.delete(`/api/grn/${encodeURIComponent(grnNo)}/`);
                            notifications.success('GRN deleted successfully.');
                            modal.hide();
                            this.loadData();
                        } catch (err) {
...
document.addEventListener('DOMContentLoaded', () => new GRNList());
