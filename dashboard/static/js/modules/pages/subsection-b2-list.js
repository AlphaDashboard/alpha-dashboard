...
// ─────────────────────────────────────────────────────────────────────────────
// SubsectionB2List — Controller class to manage the Subsection B-2 transaction list.
//
// This class loads and displays Subsection B-2 transactions in a paginated list.
// It uses:
//   - SubsectionB2API: Reusable API client to load B-2 transactions from backend.
//   - apiClient: Reusable HTTP helper to send post/get requests.
//   - domUtils: Reusable DOM helper to select elements and delegate click events.
//   - formatter: Reusable formatting helper to display dates and currencies.
//   - notifications: Reusable notification helper to show success/error toasts.
// ─────────────────────────────────────────────────────────────────────────────

class SubsectionB2List {
    ...
    constructor() {
        ...
        this.clearBtn     = domUtils.getElement('#clearFiltersBtn'); // Safe if null
        ...
        this.statusFilter = true;  // default to show active only
    ...
    bindEvents() {
        ...
        if (this.clearBtn) {
            this.clearBtn.addEventListener('click', () => {
                ...
                if (statusFilterInput) statusFilterInput.value = 'true';
                if (actionsLabel) actionsLabel.textContent = 'Active';
                this.statusFilter = true;
                this.sortField    = '';
                this.sortOrder    = '';
        ...
        domUtils.delegate('body', 'click', '.delete-btn', async (e, target) => {
            ...
            try {
                await SubsectionB2API.delete(id);
    ...
    showStatusToggleModal(id, isActive) {
        ...
        newConfirmBtn.addEventListener('click', async () => {
            ...
            try {
                await apiClient.post(`/api/subsection-b2/${id}/toggle_status/`);
    ...
    async loadData() {
        ...
        try {
            ...
            const response = await SubsectionB2API.getAll(params);
    ...
    renderTable(transactions) {
                ...
                <tr>
                    <td colspan="8" class="text-center py-5 bg-white border-0">
                        <div class="empty-state py-5 fade-in-up" style="animation-delay: 0.2s;">
                            ...
                            <p class="text-muted mb-4 fs-5">You have not created any Subsection B-2 transactions yet, or none match your search criteria.</p>
                            <a href="/subsection-b2/create/" class="btn btn-primary btn-lg shadow-sm hover-lift px-5 rounded-pill">
        ...
        let rowsHtml = transactions.map(t => {
            ...
            const isDeposit = t.rpid === 'D' || t.rpid === 'R';
            const rpidBadge = isDeposit
                ? `<span class="badge bg-success bg-opacity-10 text-success border border-success border-opacity-25 px-3 py-1 rounded-pill" style="font-size:0.75rem; font-weight: 600;">Deposit</span>`
                : `<span class="badge bg-danger bg-opacity-10 text-danger border border-danger border-opacity-25 px-3 py-1 rounded-pill" style="font-size:0.75rem; font-weight: 600;">Issue</span>`;

            let actionItemsHtml = '';
            if (t.posting_status === 'POSTED') {
                actionItemsHtml += `
                    <li>
                        <a class="dropdown-item" href="/subsection-b2/${t.voucher_no}/edit/">
                ...
                actionItemsHtml += `
                    ...
                    <li>
                        <button class="dropdown-item text-warning toggle-status-modal-btn"
