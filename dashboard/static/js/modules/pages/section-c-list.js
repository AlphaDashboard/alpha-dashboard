...
// ─────────────────────────────────────────────────────────────────────────────
// SectionCList — Architecture mirrors BankTransactionList exactly.
//
// EVENT RULES (same as BankTransactionList):
...
class SectionCList {
    ...
    bindEvents() {
        ...
        domUtils.delegate('body', 'click', '.toggle-status-btn', async (e, target) => {
            ...
            try {
                await apiClient.post(`/api/section-c/${id}/toggle_status/`);
    ...
    async loadData() {
        ...
        try {
            ...
            const response = await SectionCAPI.getAll(params);
    ...
    renderTable(transactions) {
                    ...
                    <td colspan="8" class="text-center py-5 bg-white border-0">
                        <div class="empty-state py-5 fade-in-up" style="animation-delay: 0.2s;">
                            <div class="p-4 bg-light rounded-circle d-inline-block mb-4 shadow-sm">
                                <i class="bi bi-cash-stack display-3 text-primary" style="opacity: 0.8;"></i>
                            </div>
                            <h3 class="fw-bold text-dark">No transactions found</h3>
                            <p class="text-muted mb-4 fs-5">You have not created any cash transactions yet, or none match your search criteria.</p>
                            <a href="/section-c/create/" class="btn btn-primary btn-lg shadow-sm hover-lift px-5 rounded-pill">
        ...
        let rowsHtml = transactions.map(t => {
            ...
            let actionItemsHtml = `
                    <li>
                        <a class="dropdown-item" href="/section-c/${t.voucher_no}/edit/?mode=view">
            ...
            if (isActive) {
                actionItemsHtml += `
                    <li>
                        <a class="dropdown-item" href="/section-c/${t.voucher_no}/edit/">
