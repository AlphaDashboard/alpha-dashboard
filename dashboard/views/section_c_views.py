...
class SectionCListView(TemplateView):
    template_name = 'dashboard/section_c_list.html'


class SectionCCreateView(TemplateView):
    template_name = 'dashboard/section_c_form.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        pks = list(CashBank.objects.filter(tran_type='J002', status=True).order_by('-date', '-date_created').values_list('pk', flat=True))
        current_pk = self.kwargs.get('pk', '')
        if current_pk:
            try:
                if current_pk not in pks:
                    pks = list(CashBank.objects.filter(tran_type='J002').order_by('-date', '-date_created').values_list('pk', flat=True))
